...
export default function RoutesPage() {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: 'var(--bg)' }}>
      <div style={{ padding: '54px 20px 24px', background: 'linear-gradient(180deg, #ffd0aa 0%, #ffe4d0 42%, #ffffff 100%)', marginBottom: 18, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', margin: '0 16px 18px', background: 'var(--sunk)', borderRadius: 14, padding: 4, gap: 4 }}>
        {[['discover', 'Discover'], ['mine', 'My Routes']].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)} style={{ flex: 1, padding: '10px 0', borderRadius: 11, background: tab === id ? '#fff' : 'transparent', border: 'none', color: tab === id ? 'var(--orange-deep)' : 'var(--text-soft)', fontSize: 14, fontWeight: tab === id ? 800 : 600, cursor: 'pointer', boxShadow: tab === id ? 'var(--shadow-soft)' : 'none' }}>{label}</button>
        ))}
      </div>

      {tab === 'discover' && (
        <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          {communityRoutes.map(r => <RouteCard key={r.id} route={r} onTap={() => setSelected(r)} onFollow={() => followRoute(r)} />)}
        </div>
      )}

      {tab === 'mine' && (
        <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          <button onClick={() => setShowCreate(true)} style={{ width: '100%', padding: 16, background: 'var(--orange-wash)', border: '1.5px dashed var(--orange-light)', borderRadius: 16, color: 'var(--orange-deep)', fontSize: 15, fontWeight: 800, cursor: 'pointer' }}>+ Create Route from Trip</button>
          {myRoutes.length === 0 && <div style={{ textAlign: 'center', padding: '48px 24px', color: 'var(--text-mute)' }}><div style={{ fontSize: 40, marginBottom: 12, opacity: 0.7 }}>🛣️</div><div style={{ fontSize: 14, lineHeight: 1.5 }}>No routes yet. Complete a trip and save it as a shareable route!</div></div>}
      ...
      {showCreate && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(20,20,25,0.45)', zIndex: 2000, display: 'flex', alignItems: 'flex-end', backdropFilter: 'blur(2px)' }} onClick={() => setShowCreate(false)}>
          <div style={{ width: '100%', background: 'var(--surface)', borderRadius: '24px 24px 0 0', padding: '18px 20px calc(20px + env(safe-area-inset-bottom,0px))', boxShadow: 'var(--shadow-pop)' }} onClick={e => e.stopPropagation()}>
            ...
            {trips.map(t => (
              <button key={t.id} onClick={() => handleCreateFromTrip(t)} style={{ width: '100%', padding: '14px 16px', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 13, color: 'var(--text)', fontSize: 15, fontWeight: 600, cursor: 'pointer', marginBottom: 8, textAlign: 'left', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>{t.name}</span>
...
function RouteCard({ route, onTap, onFollow }) {
  return (
    <div style={{ background: 'var(--surface)', borderRadius: 22, overflow: 'hidden', border: '1px solid var(--border)', boxShadow: 'var(--shadow-card)', cursor: 'pointer' }} onClick={onTap}>
      <div style={{ height: 118, background: `linear-gradient(150deg, #180800 0%, ${route.coverColor || '#ff7a3c'} 100%)`, position: 'relative', display: 'flex', alignItems: 'flex-end', padding: '0 15px 14px' }}>
        <div style={{ flex: 1, paddingRight: 10 }}>
          <div style={{ fontSize: 17, fontWeight: 900, color: '#fff', letterSpacing: -0.4, lineHeight: 1.2 }}>{route.name}</div>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.58)', marginTop: 3 }}>by {route.author}</div>
        </div>
        <div style={{ background: 'rgba(0,0,0,0.32)', backdropFilter: 'blur(8px)', borderRadius: 10, padding: '5px 11px', border: '1px solid rgba(255,255,255,0.14)' }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: '#fff' }}>{formatDistance(route.distance)}</div>
        </div>
        {route.isOwn && <span style={{ position: 'absolute', top: 10, right: 10, background: 'rgba(255,255,255,0.92)', color: 'var(--orange-deep)', fontSize: 9, fontWeight: 800, padding: '3px 8px', borderRadius: 20, letterSpacing: 0.5 }}>YOURS</span>}
      </div>
      <div style={{ padding: '12px 15px 14px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ flex: 1 }} />
          <button onClick={e => { e.stopPropagation(); onFollow() }} style={{ padding: '8px 18px', background: 'linear-gradient(135deg, #ff8a52, #ef5616)', border: 'none', borderRadius: 10, color: '#fff', fontSize: 12, fontWeight: 800, cursor: 'pointer', boxShadow: '0 3px 10px rgba(239,86,22,0.3)' }}>Follow</button>
        </div>
      </div>
    </div>
  )
...
function RouteDetail({ route, onBack, onFollow }) {
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: 'var(--bg)' }}>
      <div style={{ minHeight: 230, position: 'relative', overflow: 'hidden', background: `linear-gradient(165deg, #120600 0%, ${route.coverColor || '#ff7a3c'} 70%, #ef5616 100%)`, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '70px 20px 22px' }}>
        ...
        <button onClick={onBack} style={{ position: 'absolute', top: 50, left: 16, background: 'rgba(0,0,0,0.35)', backdropFilter: 'blur(10px)', border: '1px solid rgba(255,255,255,0.18)', borderRadius: 12, color: '#fff', width: 38, height: 38, fontSize: 18, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>←</button>
        <div style={{ position: 'relative', zIndex: 1 }}>
          <div style={{ fontSize: 11, color: 'rgba(255,255,255,0.55)', fontWeight: 700, letterSpacing: 1.5, textTransform: 'uppercase', marginBottom: 8 }}>by {route.author}</div>
          <h2 style={{ margin: '0 0 6px', fontSize: 28, fontWeight: 900, color: '#fff', letterSpacing: -0.7, lineHeight: 1.1 }}>{route.name}</h2>
          <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.5)' }}>{formatDate(route.createdAt)}</div>
        </div>
      </div>
      <div style={{ display: 'flex', background: 'var(--surface)', borderBottom: '1px solid var(--border)' }}>
        {[{ label: 'Distance', value: formatDistance(route.distance) }, { label: 'Duration', value: formatDuration(route.duration) }, { label: 'Places', value: route.places?.length || 0 }].map((s, i) => (
