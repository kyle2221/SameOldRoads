...
export default function HomePage() {
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: 'var(--bg)' }}>
      <div style={{ padding: '50px 20px 32px', background: 'linear-gradient(170deg, #100500 0%, #2a0e05 48%, #a03208 82%, #d44810 100%)', position: 'relative', overflow: 'hidden', minHeight: 230, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28, zIndex: 2 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 11, letterSpacing: 2, color: 'rgba(255,255,255,0.55)', fontWeight: 800, textTransform: 'uppercase' }}>Same Old Roads</span>
          </div>
          <button onClick={() => setShowProfile(p => !p)} style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg, #ff8a52, #ef5616)', border: '2px solid rgba(255,255,255,0.25)', color: '#fff', fontSize: 13, fontWeight: 800, cursor: 'pointer', boxShadow: '0 2px 12px rgba(239,86,22,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{initials}</button>
        </div>
        <div style={{ position: 'relative', zIndex: 2, flex: 1 }}>
          <div style={{ fontSize: 12, color: 'rgba(255,210,140,0.75)', fontWeight: 700, marginBottom: 8 }}>{greetIcon} {greetText}, {firstName}</div>
          <h1 style={{ margin: '0 0 10px', fontSize: 32, fontWeight: 900, color: '#fff', lineHeight: 1.06, letterSpacing: -1 }}>Where will the<br />road take you?</h1>
          <p style={{ margin: 0, fontSize: 13, color: 'rgba(255,255,255,0.45)', lineHeight: 1.6 }}>Track · Explore · Remember</p>
        </div>
      </div>

      {showProfile && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200 }} onClick={() => setShowProfile(false)}>
          <div onClick={e => e.stopPropagation()} style={{ position: 'absolute', top: 86, right: 16, background: '#fff', borderRadius: 20, boxShadow: '0 12px 48px rgba(0,0,0,0.18)', border: '1px solid var(--border)', padding: '18px 20px', minWidth: 228, zIndex: 201 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg, #ff8a52, #ef5616)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: 16 }}>{initials}</div>
              <div>
            ...
            <button onClick={() => { setShowProfile(false); logout() }} style={{ width: '100%', padding: '11px 0', borderRadius: 13, background: '#fff8f6', border: '1px solid var(--orange-tint)', color: 'var(--orange-deep)', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>Sign Out</button>
          </div>
        </div>
      )}
...
function QuickBtn({ icon, label, sub, onClick, accent }) {
  return (
    <button onClick={onClick} style={{ flex: 1, padding: '16px 8px 14px', background: accent ? 'linear-gradient(145deg, #ff8a52, #ef5616)' : 'var(--surface)', border: `1px solid ${accent ? 'transparent' : 'var(--border)'}`, borderRadius: 20, color: accent ? '#fff' : 'var(--text)', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, boxShadow: accent ? '0 6px 22px rgba(239,86,22,0.36)' : 'var(--shadow-soft)' }}>
  ...
  return (
    <div style={{ marginBottom: 28 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 16px', marginBottom: 14 }}>
        ...
        {action && <button onClick={onAction} style={{ background: 'none', border: 'none', color: 'var(--orange-deep)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>{action} →</button>}
      </div>
      {children}
    </div>
  )
...
function HomeRouteCard({ route, onFollow }) {
  return (
    <div style={{ minWidth: 200, borderRadius: 20, overflow: 'hidden', border: '1px solid var(--border)', boxShadow: 'var(--shadow-card)', flexShrink: 0, background: 'var(--surface)' }}>
      <div style={{ height: 108, position: 'relative', overflow: 'hidden', background: `linear-gradient(150deg, #180800 0%, ${route.coverColor || '#ff7a3c'} 100%)`, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '10px 13px' }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 900, color: '#fff', letterSpacing: -0.4, lineHeight: 1.2, textShadow: '0 1px 8px rgba(0,0,0,0.5)' }}>{route.name}</div>
        <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.55)', marginTop: 2 }}>by {route.author}</div>
      </div>
      <div style={{ padding: '10px 12px' }}>
        <button onClick={onFollow} style={{ width: '100%', padding: '9px 0', background: 'var(--orange-wash)', border: '1px solid var(--orange-tint)', borderRadius: 11, color: 'var(--orange-deep)', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>Follow Route →</button>
      </div>
    </div>
  )
}
...
function TripRow({ trip, last, places }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', padding: '14px 16px', borderBottom: last ? 'none' : '1px solid var(--border-soft)', gap: 13 }}>
      <div style={{ width: 44, height: 44, borderRadius: 14, background: 'linear-gradient(145deg, #fff1e8, #ffe0cc)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>🚗</div>
