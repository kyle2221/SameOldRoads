  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', background: 'var(--bg)' }}>
      <div style={{ padding: '54px 20px 22px', background: 'linear-gradient(180deg, #ffd0aa 0%, #ffe4d0 42%, #ffffff 100%)', position: 'relative', overflow: 'hidden' }}>
        ...
        <p style={{ margin: 0, fontSize: 14, color: 'var(--text-soft)', position: 'relative' }}>{restaurants.length} restaurants · {destinations.length} destinations</p>
      </div>

      <div style={{ padding: '16px 16px 12px' }}>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍  Search places..." style={{ width: '100%', padding: '13px 16px', borderRadius: 14, background: 'var(--surface-2)', border: '1px solid var(--border)', color: 'var(--text)', fontSize: 15, outline: 'none' }} />
      </div>
      ...
      <div style={{ display: 'flex', margin: '0 16px 16px', background: 'var(--sunk)', borderRadius: 14, padding: 4, gap: 4 }}>
        {[['all', 'All'], ['restaurant', '🍽️ Food'], ['destination', '📍 Spots']].map(([id, label]) => (
          <button key={id} onClick={() => setFilter(id)} style={{ flex: 1, padding: '10px 0', borderRadius: 11, background: filter === id ? '#fff' : 'transparent', border: 'none', color: filter === id ? 'var(--orange-deep)' : 'var(--text-soft)', fontSize: 13, fontWeight: filter === id ? 800 : 600, cursor: 'pointer', boxShadow: filter === id ? 'var(--shadow-soft)' : 'none' }}>{label}</button>
        ))}
      </div>
      ...
      <div style={{ padding: '0 16px', display: 'flex', flexDirection: 'column', gap: 11 }}>
        ...
        {filtered.map(place => (
          <PlaceCard key={place.id} place={place} editing={editing === place.id} editData={editData} setEditData={setEditData} onEdit={() => startEdit(place)} onSave={saveEdit} onCancel={() => setEditing(null)} onDelete={() => deletePlace(place.id)} />
        ))}
      </div>
      <div style={{ height: 24 }} />
    </div>
      ...
      <div style={{ background: 'var(--surface)', borderRadius: 18, padding: 16, border: '1.5px solid var(--orange)', boxShadow: 'var(--shadow-card)' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          {['restaurant', 'destination'].map(t => (
            <button key={t} onClick={() => setEditData(d => ({ ...d, type: t }))} style={{ flex: 1, padding: '9px 0', borderRadius: 10, background: editData.type === t ? 'var(--orange-wash)' : 'var(--surface-2)', border: `1.5px solid ${editData.type === t ? 'var(--orange)' : 'var(--border)'}`, color: editData.type === t ? 'var(--orange-deep)' : 'var(--text-soft)', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>{t === 'restaurant' ? '🍽️ Restaurant' : '📍 Destination'}</button>
          ))}
        </div>
      ...
      <div style={{ flex: 1, padding: '14px 15px', display: 'flex', gap: 13 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            ...
            <span style={{ fontSize: 10, background: 'var(--sunk)', borderRadius: 6, padding: '2px 8px', color: 'var(--text-soft)', flexShrink: 0, marginLeft: 8, fontWeight: 700, letterSpacing: 0.3 }}>{place.type}</span>
          </div>
          {place.notes ? <div style={{ fontSize: 13, color: 'var(--text-soft)', marginBottom: 10, lineHeight: 1.5 }}>{place.notes}</div> : <div style={{ fontSize: 13, color: 'var(--text-mute)', marginBottom: 10, fontStyle: 'italic' }}>No notes yet</div>}
          <div style={{ display: 'flex', gap: 8 }}>
