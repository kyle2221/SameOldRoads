  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const map = L.map(el, {
        ...
        doubleClickZoom: interactive, touchZoom: interactive, tap: interactive,
      })
      mapRef.current = map
      ...
      const drawLine = (latlngs) => {
        ...
        const fit = () => { if (cancelled || !mapRef.current) return; map.invalidateSize(); if (fitTarget.getLayers().length) map.fitBounds(fitTarget.getBounds(), { padding: [30, 30], maxZoom: 13 }) }
