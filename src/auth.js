...
export function getCurrentUser() { try { return JSON.parse(localStorage.getItem(SESSION_KEY) || 'null') } catch { return null } }
...
export async function signInWithEmail(email, password) {
  ...
  if (user.pw !== btoa(unescape(encodeURIComponent(password)))) throw new Error('Incorrect password.')
  return storeSession({ id: user.id, email: user.email, name: user.name, provider: 'email' })
}
